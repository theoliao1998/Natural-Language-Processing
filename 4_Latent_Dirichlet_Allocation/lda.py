...
class LdaTopicCounts:
    ...
    def report(self, vocab, handle, limit=25):
        ...
        for kk in self._normalizer:
            ...
            for ww in sorted(self._topic_term[kk],key= lambda x: -self.word_in_topic(kk,x)):
...
class Sampler:
    ...
    def report_topics(self, vocab, outputfilename, limit=10):
        ...
        """
        topicsfile = open(outputfilename + ".topics", 'w', encoding="utf8")
        self._topics.report(vocab, topicsfile, limit)

        statsfile = open(outputfilename + ".stats", 'w', encoding="utf8")
        ...
        topicassignfile = open(outputfilename + ".topic_assign", 'w', encoding="utf8")
        ...
        doctopicsfile = open(outputfilename + ".doc_topics", 'w', encoding="utf8")
    ...
    def sample_probs(self, doc_id, index):
        ...
        doc_term_num = sum(self._doc_counts[doc_id].values())
        # doc_term_num = len(self._doc_assign[doc_id]) - 1
        ...
        for kk in range(self._num_topics):
            ...
            # uniform probability.
            sample_probs[kk] = self._topics.word_in_topic(kk,term) *\
                ((self._doc_counts[doc_id][kk] + self._alpha[kk]) / (doc_term_num + alpha_sum))
...
if __name__ == "__main__":
    ...
    lda.report_topics(vocab, args.output, 49)
